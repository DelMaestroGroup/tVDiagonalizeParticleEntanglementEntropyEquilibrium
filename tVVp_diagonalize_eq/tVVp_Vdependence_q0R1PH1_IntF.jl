...
Usage: 
    tVVp_Vdependence_q0R1PH1_IntF.jl [--out FOLDER] [--tmp FOLDER]
                            [--out-obdm FOLDER] [--g2] [--obdm]
                            [--spatial] [--skip-hoffdiag-saving]
                            [--skip-hoffdiag-loading] [--no-flush]
                            [--no-recompute-structure-matrix] [--pbc]
                            [--obc] [--V-start V_start] [--V-end V_end]
                            [--V-step V_step] [--V-list [V_LIST...]]
                            [--Vp Vp] [--t t] --ee ℓ [-h] M N
    ...
    tV parameters:
            ...
            --V-list [V_LIST...]  multiple V values used as V_array and ignore
                                    V-start, V-end, V-step (type: Float64)
...
function main()
    ...
    if ~(c[:V_list] === nothing) && length(c[:V_list]) > 0
        ...
        # sorry that is a quick fix, should put some thoughts into it later
